...
A Next.js storefront demonstrating the full [Stripe Checkout](https://stripe.com/docs/payments/checkout) flow against the local Stripe API from `api-emulator`.
...
1. The home page fetches products and prices from the local Stripe API via the official `stripe` SDK
...
4. The browser redirects to the local checkout page where the user clicks "Pay and Complete"
...
The Stripe SDK is configured with `host: localhost` and `protocol: http` so all API calls stay local. A thin proxy route at `/v1/[...path]` forwards SDK requests to the embedded emulator at `/api-emulator/stripe/v1/*`.
...
| Product | Price |
|---------|-------|
| API Emulator T-Shirt | $25.00 |
| API Emulator Mug | $15.00 |
| API Emulator Sticker Pack | $8.00 |
| API Emulator Hoodie | $50.00 |
