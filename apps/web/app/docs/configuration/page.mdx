...
Configuration is optional. All services start with sensible defaults. To customize seed data, create `api-emulator.config.yaml` in your project root (or pass `--seed`):
...
Check a plugin shelf entry or direct plugin file before installing or loading it:
...
`api list` and `api install` surface provider fidelity as `contract-backed`, `smoke-only`, `stub`, or `generated fallback` when plugin metadata is available.

Use `--skip-build` or `--skip-load` when validating source-only local plugins.
...
The scaffold includes starter fidelity metadata and is tracked in `.api-emulator/manifest.json` so reruns do not overwrite local edits unless you pass `--yes`. `api plugin create` remains available as a compatibility alias.
...
If no `oauth_apps` are configured, the emulator accepts any `client_id`. With apps configured, strict validation is enforced.
...
Default seed (always created): S3 bucket `api-emulator-default`, SQS queue `api-emulator-default-queue`, IAM user `admin` with access key pair.
