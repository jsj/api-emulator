...
`api list` and `api plugin install` show provider fidelity when plugin metadata is available.
...
The starter files include fidelity metadata. The CLI does not replace changed generated files unless you use `--yes`.
